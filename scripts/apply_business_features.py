...
def replace(path, old, new):
    ...
    if old in text: p.write_text(text.replace(old, new, 1), encoding='utf-8')
...
if 'import com.example.util.PurchaseReceiptFormatter' not in t: t = t.replace('import com.example.util.PrinterConnectionState', 'import com.example.util.PrinterConnectionState\nimport com.example.util.PurchaseReceiptFormatter')
...
old_callback = 'viewModel.createPurchaseInvoice(invoice, resolved) { showCreate = false; Toast.makeText(context, "تم حفظ الفاتورة وإضافة الأصناف الجديدة إلى المخزون", Toast.LENGTH_LONG).show() }'
new_callback = 'viewModel.createPurchaseInvoice(invoice, resolved) { id -> showCreate = false; selectedPurchase = invoice.copy(id = id); Toast.makeText(context, "تم حفظ فاتورة المورد بنجاح", Toast.LENGTH_LONG).show() }'
t = t.replace(old_callback, new_callback, 1)
