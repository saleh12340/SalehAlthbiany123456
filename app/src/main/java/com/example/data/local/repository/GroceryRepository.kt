...
class GroceryRepository(private val dao: GroceryDao) {
    ...
    /** Delete the account without deleting historical invoices. The invoices are detached,
     * while the customer's account movements are removed. */
    ...
    /** Delete one customer movement and reverse the financial effect that created it. */
    suspend fun deleteCustomerTransaction(transaction: CustomerTransaction) {
        if (transaction.invoiceId != null) {
            dao.updateCustomerFinancials(
                id = transaction.customerId,
                delta = -transaction.remaining,
                salesDelta = -transaction.amount,
                paidDelta = -transaction.paid
            )
        } else {
            dao.updateCustomerFinancials(
                id = transaction.customerId,
                delta = transaction.paid,
                salesDelta = 0.0,
                paidDelta = -transaction.paid
            )
        }
