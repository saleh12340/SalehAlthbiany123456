...
interface GroceryDao {
    ...
    @Query("SELECT * FROM products WHERE name LIKE '%' || :query || '%' OR barcode LIKE '%' || :query || '%' ORDER BY name ASC") fun searchProducts(query: String): Flow<List<Product>>
    ...
    @Query("UPDATE products SET quantity = quantity + :delta, updatedAt = :timestamp WHERE id = :id") suspend fun updateProductStock(id: Long, delta: Double, timestamp: Long = System.currentTimeMillis())
    @Delete suspend fun deleteProduct(product: Product)
    @Query("DELETE FROM products WHERE id = :id") suspend fun deleteProductById(id: Long)

    @Query("SELECT * FROM customers ORDER BY name ASC") fun getAllCustomers(): Flow<List<Customer>>
    @Query("SELECT * FROM customers WHERE name LIKE '%' || :query || '%' OR phone LIKE '%' || :query || '%' ORDER BY name ASC") fun searchCustomers(query: String): Flow<List<Customer>>
    @Query("SELECT * FROM customers WHERE id = :id") suspend fun getCustomerById(id: Long): Customer?
    @Query("SELECT * FROM customers WHERE balance > 0 ORDER BY balance DESC") fun getCustomersWithDebts(): Flow<List<Customer>>
    @Query("SELECT COALESCE(SUM(balance), 0.0) FROM customers WHERE balance > 0") fun getTotalCustomerDebts(): Flow<Double>
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertCustomer(customer: Customer): Long
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertCustomers(customers: List<Customer>)
    @Update suspend fun updateCustomer(customer: Customer)
    @Query("UPDATE customers SET balance = balance + :delta, totalSales = totalSales + :salesDelta, totalPaid = totalPaid + :paidDelta WHERE id = :id") suspend fun updateCustomerFinancials(id: Long, delta: Double, salesDelta: Double = 0.0, paidDelta: Double = 0.0)
    @Delete suspend fun deleteCustomer(customer: Customer)
    @Query("DELETE FROM customer_transactions WHERE customerId = :customerId") suspend fun deleteCustomerTransactions(customerId: Long)
    @Query("UPDATE sale_invoices SET customerId = NULL, customerName = 'عميل محذوف', customerPhone = '' WHERE customerId = :customerId") suspend fun detachCustomerFromInvoices(customerId: Long)

    @Query("SELECT * FROM suppliers ORDER BY name ASC") fun getAllSuppliers(): Flow<List<Supplier>>
    @Query("SELECT * FROM suppliers WHERE name LIKE '%' || :query || '%' OR company LIKE '%' || :query || '%' OR phone LIKE '%' || :query || '%' ORDER BY name ASC") fun searchSuppliers(query: String): Flow<List<Supplier>>
    @Query("SELECT * FROM suppliers WHERE id = :id") suspend fun getSupplierById(id: Long): Supplier?
    @Query("SELECT COALESCE(SUM(balance), 0.0) FROM suppliers WHERE balance > 0") fun getTotalSupplierDebts(): Flow<Double>
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertSupplier(supplier: Supplier): Long
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertSuppliers(suppliers: List<Supplier>)
    @Update suspend fun updateSupplier(supplier: Supplier)
    @Query("UPDATE suppliers SET balance = balance + :delta, totalPurchases = totalPurchases + :purchasesDelta, totalPaid = totalPaid + :paidDelta WHERE id = :id") suspend fun updateSupplierFinancials(id: Long, delta: Double, purchasesDelta: Double = 0.0, paidDelta: Double = 0.0)
    @Delete suspend fun deleteSupplier(supplier: Supplier)

    @Query("SELECT * FROM sale_invoices ORDER BY timestamp DESC") fun getAllSaleInvoices(): Flow<List<SaleInvoice>>
    @Query("SELECT * FROM sale_invoices WHERE invoiceNumber LIKE '%' || :query || '%' OR customerName LIKE '%' || :query || '%' ORDER BY timestamp DESC") fun searchSaleInvoices(query: String): Flow<List<SaleInvoice>>
    @Query("SELECT * FROM sale_invoices WHERE date = :date ORDER BY timestamp DESC") fun getSaleInvoicesByDate(date: String): Flow<List<SaleInvoice>>
    @Query("SELECT * FROM sale_invoices WHERE date BETWEEN :startDate AND :endDate ORDER BY timestamp DESC") fun getSaleInvoicesBetweenDates(startDate: String, endDate: String): Flow<List<SaleInvoice>>
    @Query("SELECT * FROM sale_invoices WHERE id = :id") suspend fun getSaleInvoiceById(id: Long): SaleInvoice?
    @Query("SELECT * FROM sale_invoices WHERE customerId = :customerId ORDER BY timestamp DESC") fun getSaleInvoicesForCustomer(customerId: Long): Flow<List<SaleInvoice>>
    @Query("SELECT COUNT(*) FROM sale_invoices") suspend fun getSaleInvoiceCount(): Int
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertSaleInvoice(invoice: SaleInvoice): Long
    @Delete suspend fun deleteSaleInvoice(invoice: SaleInvoice)
    @Query("SELECT * FROM sale_invoice_items WHERE invoiceId = :invoiceId") fun getItemsForSaleInvoice(invoiceId: Long): Flow<List<SaleInvoiceItem>>
    @Query("SELECT * FROM sale_invoice_items WHERE invoiceId = :invoiceId") suspend fun getItemsForSaleInvoiceList(invoiceId: Long): List<SaleInvoiceItem>
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertSaleInvoiceItems(items: List<SaleInvoiceItem>)
    @Query("DELETE FROM sale_invoice_items WHERE invoiceId = :invoiceId") suspend fun deleteSaleInvoiceItems(invoiceId: Long)
    @Query("DELETE FROM customer_transactions WHERE invoiceId = :invoiceId") suspend fun deleteCustomerTransactionForInvoice(invoiceId: Long)

    @Query("SELECT * FROM purchase_invoices ORDER BY timestamp DESC") fun getAllPurchaseInvoices(): Flow<List<PurchaseInvoice>>
    @Query("SELECT * FROM purchase_invoices WHERE invoiceNumber LIKE '%' || :query || '%' OR supplierName LIKE '%' || :query || '%' ORDER BY timestamp DESC") fun searchPurchaseInvoices(query: String): Flow<List<PurchaseInvoice>>
    ...
    @Query("SELECT * FROM purchase_invoice_items WHERE invoiceId = :invoiceId") fun getItemsForPurchaseInvoice(invoiceId: Long): Flow<List<PurchaseInvoiceItem>>
    ...
    @Query("SELECT * FROM customer_transactions WHERE id = :id") suspend fun getCustomerTransactionById(id: Long): CustomerTransaction?
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertCustomerTransaction(transaction: CustomerTransaction): Long
    @Update suspend fun updateCustomerTransaction(transaction: CustomerTransaction)
    ...
    @Query("SELECT * FROM supplier_transactions WHERE id = :id") suspend fun getSupplierTransactionById(id: Long): SupplierTransaction?
    ...
    @Update suspend fun updateSupplierTransaction(transaction: SupplierTransaction)
    ...
    @Query("SELECT * FROM expenses WHERE date = :date ORDER BY timestamp DESC") fun getExpensesByDate(date: String): Flow<List<Expense>>
    @Query("SELECT * FROM expenses WHERE date BETWEEN :startDate AND :endDate ORDER BY timestamp DESC") fun getExpensesBetweenDates(startDate: String, endDate: String): Flow<List<Expense>>
    @Query("SELECT COALESCE(SUM(amount), 0.0) FROM expenses WHERE date = :date") fun getTodayExpensesSum(date: String): Flow<Double>
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertExpense(expense: Expense): Long
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertExpenses(expenses: List<Expense>)
